...
createTime: 2025/12/24 16:17:55
---

::: caution
如果你感觉你的浏览器异常的卡顿, 请关闭该页面, 该篇文档写了大量的图表, 可能会导致浏览器卡顿.

不过加载完就不卡了, 一次性加载太多才卡的.
:::

[echarts](https://echarts.apache.org/zh/index.html)是一个功能强大的图表库.

如果你觉得太复杂, 不会使用, 建议使用下一篇文档的组件[chart.js](chartjs.md).

图表配置请查看[ECharts文档](https://echarts.apache.org/handbook/zh/get-started/).

图表官方示例请查看[ECharts示例](https://echarts.apache.org/examples/zh/index.html).

## 语法

### JSON 配置

如果你可以很轻松的生成数据, 你可以直接通过一个JSON代码块来提供Echarts配置
...
如果你需要通过脚本来获取数据, 你可以使用javascript代码块提供Echarts配置.

我们将通过`echarts`变量暴露ECharts实例, 并且你应该将ECharts配置赋值给`option`变量.
